...
#TODO: update with aligned years, 2001 to end of 2020 (2001-01-03 to 2021-01-06)
pred.raw <- pred.raw[pred.raw$date >= "2001-01-03" & pred.raw$date <= "2021-01-06", ]
#pred.raw <- pred.raw[pred.raw$date <= "2021-01-06", ] #old version, keep until figure is finalized
...
#yearly ticks
x.ticks.resp <- seq(year(resp.time.range[1]), year(resp.time.range[2]), by = 1)
x.ticks.resp <- ymd(paste0(x.ticks.resp, "01", "01"))
x.resp.reduced <- x.ticks.resp[1:20]

time.resp.plot <- as.Date(resp.time)
...
#updated figure 1 time series, 
## resp SE Aus
## pred Nino, WTIO, ETIO, TSA, SAM, OLR

#updated y.ticks
#response setup:
y.SE.max <-  round(max(SE.anom)/10)*10
y.SE.min <- round(min(SE.anom)/10)*10
y.SE.ticks <- c(-20, 0, 25, 50)
#y.SE.ticks <- seq(y.SE.min, y.SE.max, by = 20)
...
#resp
#SE Aus:
over.seaus <- SE.anom >= 0
seaus.top <- SE.anom
seaus.top[!over.seaus] <- 0
seaus.bot <- SE.anom
seaus.bot[over.seaus] <- 0
...
png(filename = "SEresp_pred_ts.png", width = 4600, height = 5400, res = 250)
par(mfrow = c(7, 1))
...
axis(side = 2, at = y.SE.ticks, cex.axis = 2.25, 
     col = NA, line = 0,
     col.ticks = "black", col.axis = "black", las =1)
mtext("CO Anomaly [ppb]", side = 2, line = 4,  cex = 2)
abline(v = x.ticks.resp[1:(length(x.ticks.resp))],
       lty = 2, col = "grey", lwd = 2)
abline(h = 0, lty = 1, col = "grey", lwd = 1)
envelopePlot(x1 = time.resp.plot,
...
envelopePlot(x1 = time.resp.plot,
             ...
             col = alpha(bot.col.resp, 0.67),
             lineCol = NA)
legend(x = ymd("2000-10-01"), #adjust according to final .png output
       y = c(56),
       legend = "Southeast Australia", #change to Southeast Australia (or nothing)
       box.col = NA, bg = NA,
