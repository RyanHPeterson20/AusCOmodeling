...
#
# Symbol x-position derivation (general form, seg.len = 2 fixed):
#   cw       = (text_x - box_left) / (2 * x.intersp + seg.len)
#   sym_ctr  = box_left + (x.intersp + seg.len/2) * cw
#   seg_half = seg.len/2 * cw
#   At the default x.intersp = 1, seg.len = 2 these reduce to the
#   special-case expressions: sym_ctr = (box_left + text_x) / 2,
#   seg_half = (text_x - box_left) / 4.
#
.draw_model_legend <- function(pos, inset, title_str, cex,
                                labels, pch_val, lty_vec, lwd_val,
                                cols, pt_cex, x_intersp = 1) {
  ...
  # x.intersp must match here so the bounding box and text$x positions are
  # computed with the same spacing that will be used when drawing.
  ...
  # General geometry (seg.len = 2 fixed; x.intersp variable):
  #   cw       = (text_x - box_left) / (2 * x_intersp + 2)
  #   sym_ctr  = box_left + (x_intersp + 1) * cw
  #   seg_half = cw
  cw       <- (lg_with$text$x[1L] - box_left) / (2 * x_intersp + 2)
  sym_x    <- box_left + (x_intersp + 1) * cw
  seg_half <- cw
...
plot_lagged_coef_panels <- function(
  ...
  # Gap between the legend symbol and the label text, in character widths.
  # Passed as x.intersp to legend().  Default 1 = R's standard spacing.
  # Increase to widen the gap; decrease (e.g. 0.5) to tighten it.
  legend_x_intersp_terms = 1,
  legend_x_intersp_model = 1,
  ...
  if (isTRUE(add_legends)) {
    ...
    legend(legend_pos_terms,
           ...
           x.intersp = legend_x_intersp_terms)
    ...
    .draw_model_legend(
      ...
      pt_cex    = 2.25,
      x_intersp = legend_x_intersp_model)
  }
