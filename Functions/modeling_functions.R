...
pred_setup <- function(aus.lag, season.weeks, early, mid, late, j = 1:20){
  
  early.ind <- which(season.weeks %in% early)
  mid.ind <- which(season.weeks %in% mid)
  late.ind <- which(season.weeks %in% late)
  
  temp.lag <- lapply(aus.lag, function(x) x[j, -c(1:2)] )
  
  #extract preds
  early.preds <- do.call(rbind, temp.lag[early.ind])
  mid.preds <- do.call(rbind, temp.lag[mid.ind])
  late.preds <- do.call(rbind, temp.lag[late.ind])
  

  return(list(early = early.preds, mid = mid.preds, late = late.preds))
}


resp_setup <- function(resp.mat, season.weeks, early, mid, late, j = 1:20){
  
  early.ind <- which(season.weeks %in% early)
  mid.ind <- which(season.weeks %in% mid)
  late.ind <- which(season.weeks %in% late)
  
  return(list(early = as.vector(resp.mat[j ,early.ind]),
        mid = as.vector(resp.mat[j ,mid.ind]),
        late = as.vector(resp.mat[j ,late.ind])))
