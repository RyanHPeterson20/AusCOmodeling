...
#SE.pred <- lapply(SE.pred, scale)


all_vals    <- unlist(SE.pred)
global_mean <- mean(all_vals, na.rm = TRUE)
global_sd   <- sd(all_vals, na.rm = TRUE)

SE.pred <- lapply(SE.pred, function(x) {
...
save(SEmodels, SErefits , 
     SE.vary.terms, SE.const.LM, SE.vary.LM,
     file = "std_RAMPmodels.rda")
